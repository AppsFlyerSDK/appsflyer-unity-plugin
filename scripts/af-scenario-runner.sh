...
ios_collect_logs() {
  ...
  if [[ -d "$sim_data_dir" ]]; then
    local qa_log
    # Use the most recently modified af_qa_logs.txt — multiple stale containers can
    # accumulate after repeated installs and the oldest one must not poison results.
    qa_log=$(find "$sim_data_dir/Containers/Data/Application" -name "af_qa_logs.txt" -maxdepth 4 2>/dev/null | \
...
validate_check() {
  ...
  case "$check_type" in
    ...
    count_matches)
      ...
      local count
      count=$(grep -cE "$pattern" "$log_file" 2>/dev/null) || count=0
