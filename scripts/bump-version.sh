...
# This file is only regenerated by the Android Resolver when it runs inside the
# Editor UI - CI's batch-mode builds never trigger it, so if this script doesn't
# also patch the already-resolved coordinates here, a version bump silently
# leaves CI building against the old af-android-plugin-bridge/unity-wrapper AARs
# (see commit 45df2922 and the unity-wrapper 7.0.14 drift it took two more bumps
# to notice).
MAIN_GRADLE="test-app/Assets/Plugins/Android/mainTemplate.gradle"
if [[ -f "$MAIN_GRADLE" ]]; then
  echo "[11/11] $MAIN_GRADLE — af-android-plugin-bridge, unity-wrapper"
  ...
  sed -i.bak "s|com.appsflyer:unity-wrapper:[^']*|com.appsflyer:unity-wrapper:$UNITY_WRAPPER_VERSION|" "$MAIN_GRADLE"
