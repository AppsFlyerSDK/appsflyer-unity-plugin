...
namespace AppsFlyerSDK
{
    ...
    {
        public static readonly string kAppsFlyerPluginVersion = "7.0.1-rc1";
        ...
        public static void initSDK(string devKey, string appID, MonoBehaviour gameObject)
        {
            ...
            if (instance == null || !instance.isInit)
            {
                ...
                // Register the native deep-link listener before the "init" RPC call
                // below fires unifiedDeepLinking(). The native SDK only enqueues the
                // clean-launch DDL check if a listener is already registered at that
                // exact moment (F-094-deferred-deep-linking.md) — it's a one-shot gate,
                // not a retry, so subscribing later (e.g. only when the app adds an
                // OnDeepLinkReceived handler) can permanently miss the first-launch
                // NOT_FOUND/FOUND callback.
